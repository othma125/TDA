...
class math_model:
    def __init__(self, input_data: data):
        ...
        for t in self.__inputs.trains:
            departure_time: int = t.departure_time if t.departure_time % self.__inputs.time_step == 0 else self.__inputs.time_step * ceil(t.departure_time / self.__inputs.time_step)
            ...
            for time in range(departure_time, max_time_stamp, self.__inputs.time_step):
                for tr in t.tracks:
                    ...
                    self.__travel_arc_variables["t".join(tr_arc.get_unique_key())] = tr_arc.get_binary_variable()
                for loc in t.route:
                    if loc == t.arrival_location or loc == t.departure_location:
                        continue
                    w_arc: waiting_arc = waiting_arc(t, time, loc)
                    self.__waiting_arc_variables["w".join(w_arc.get_unique_key())] = w_arc.get_binary_variable_for_waiting()
            ...
            for time in range(departure_time, max_time_stamp, self.__inputs.time_step):
                for tr in t.tracks:
                    if tr.arrival_location == t.arrival_location:
                        ...
                        obj += self.__travel_arc_variables['t'.join(tr_arc.get_unique_key())] * (time_stamp - t.arrival_time) * t.category
                ...
                for tr in t.tracks:
                    if tr.departure_location == t.departure_location:
                        ...
                        if time == departure_time:
                            self.__model += self.__travel_arc_variables[uni_key] == 1
            ...
            for tr in t.tracks:
                ...
                arrival = self.__inputs.time_step * ceil(arrival / self.__inputs.time_step) if arrival % self.__inputs.time_step > 0 else arrival
        ...
        for key in self.__travel_arc_variables.keys():
            ...
            arrival = arrival if arrival % self.__inputs.time_step == 0 else self.__inputs.time_step * ceil(arrival / self.__inputs.time_step)
            for t in self.__inputs.trains:
                if t == tr_arc.train:
                    continue
