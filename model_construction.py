        ...
        for t in self.inputs.trains:
            for time in range(departure_time, max_time_stamp, self.inputs.time_step):
                for tr in t.tracks:
                    ...
                    self.travel_arc_variable["t".join(tr_arc.get_unique_key())] = tr_arc.get_binary_variable()
                for loc in t.route:
                    if loc == t.arrival_location or loc == t.departure_location:
                        continue
                    w_arc: waiting_arc = waiting_arc(t, time, loc)
                    self.waiting_arc_variable["w".join(w_arc.get_unique_key())] = w_arc.get_binary_variable_for_waiting()
            ...
            for time in range(departure_time, max_time_stamp, self.inputs.time_step):
                for tr in t.tracks:
                    if tr.arrival_location == t.arrival_location:
                        ...
                        obj += self.travel_arc_variable['t'.join(tr_arc.get_unique_key())] * (time_stamp - t.arrival_time) * t.category

            constraint_value2 = 0
            constraint_value3 = 0
            constraint_value22 = 0
            constraint_value33 = 0
            for time in range(departure_time, max_time_stamp, self.inputs.time_step):
                for tr in t.tracks:
                    if tr.departure_location == t.departure_location:
                        ...
                        if time >= t.departure_time:
                            constraint_value2 += self.travel_arc_variable[uni_key]
                        else:
                            constraint_value22 += self.travel_arc_variable[uni_key]
                    elif tr.arrival_location == t.arrival_location:
                        tr_arc: travel_arc = travel_arc(t, time, tr)
                        uni_key: str = 't'.join(tr_arc.get_unique_key())
                        if time >= t.departure_time:
                            constraint_value3 += self.travel_arc_variable[uni_key]
                        else:
                            constraint_value33 += self.travel_arc_variable[uni_key]
            # constraint 2
            ...
            for time in range(t.departure_time, max_time_stamp, self.inputs.time_step):
                for tr in t.tracks:
                    if tr.arrival_location == t.arrival_location:
                        continue
                    tr_arc: travel_arc = travel_arc(t, time, tr)
                    uni_key = 't'.join(tr_arc.get_unique_key())
                    sum1 = self.travel_arc_variable[uni_key]
                    time_stamp: int = time + tr.traveled_time(self.inputs.trains_speed)
                    ar_arc: waiting_arc = waiting_arc(t, self.inputs.time_step * (time_stamp // self.inputs.time_step), tr.arrival_location)
                    uni_key2 = 'a'.join(ar_arc.get_unique_key())
                    ...
                    uni_key2 = 'w'.join(w_arc.get_unique_key())
                    sum1 += self.waiting_arc_variable[uni_key2] if uni_key2 in self.waiting_arc_variable else 0
                    sum2 = 0
                    ...
                    uni_key = 'w'.join(w_arc.get_unique_key())
                    sum2 += self.waiting_arc_variable[uni_key] if uni_key in self.waiting_arc_variable.keys() else 0
                    for tr2 in t.tracks:
                        if tr2.departure_location == tr.arrival_location:
                            tr_arc2: travel_arc = travel_arc(t, time_stamp + self.inputs.time_step, tr2)
                            uni_key = 't'.join(tr_arc2.get_unique_key())
                            sum2 += self.travel_arc_variable[uni_key] if uni_key in self.travel_arc_variable.keys() else 0
                    self.model += sum1 == sum2
        self.model += obj
        # waiting sites capacity constraint
        for key, x in self.waiting_arc_variable.items():
            ...
            for t in self.inputs.trains:
                if t != w_arc.train:
                    ...
                    if uni_key in self.waiting_arc_variable.keys():
                        c = True
                        constraint_value += self.waiting_arc_variable[uni_key]
                ...
                if t != tr_arc.train:
                    for time in range(tr_arc.time_stamp + self.inputs.time_step, max_time_stamp, self.inputs.time_step):
                        if time - tr_arc.time_stamp <= travel_time:
                            ...
                            if uni_key in self.travel_arc_variable.keys():
                                c = True
                                constraint_value += self.travel_arc_variable[uni_key]
